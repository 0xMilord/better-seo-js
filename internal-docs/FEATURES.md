...
| N4 | **JSON-LD in Next output** | `metadata` / `other` JSON-LD fields | `@better-seo/next` uses **`serializeJSONLD`** from core | Single serializer; Next adapter does not reimplement stringify. |

...
| **🟨 Partial**                    | **9, 12**        | Meaningful pieces exist; **Roadmap exit criteria** for that wave are **not** fully met.                                                                                                                                                                                                                     |
| **⬜ Not started** (or stub only) | **7, 8, 10, 11** | No / negligible implementation toward that wave’s **primary** deliverables.                                                                                                                                                                                                                                 |
...
| ------ | -------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| **9**  | Clack **TUI** launcher, CLI **`crawl`**, **`doctor`**, **`init`**, **`migrate`** hints, **`docs/commands.md`** | Industry **templates** (**L9**), **`template switch` / `preview`** (**L10–L11**), deeper **`doctor`** (adapter/config CI) (**L8**) |
| **12** | **`better-seo-crawl`**, CLI \*\*`crawl robots                                                                  | sitemap`**, **`fromNextSeo`\*\* baseline                                                                                           | **RSS/Atom**, **`llms.txt`**, **sitemap index / hreflang** depth, **`migrate`** **codemods** (**L7**), docs finish per roadmap |

### ⬜ Not started — main work ahead

| Wave   | Theme                  | Main deliverables (from **`Roadmap.md`**)                                                                      |
| ------ | ---------------------- | -------------------------------------------------------------------------------------------------------------- |
| **7**  | Content → SEO          | **`fromContent`**, **`fromMDX` / compiler package** (**C16/C17**)                                              |
| **8**  | Trust / debug CLI      | **`snapshot`**, **`preview`** (**L5/L6**)                                                                      |
| **10** | Automation             | **`add`**, **`scan`**, **`fix`**, **`analyze`** (**L3/L4**) — **blocked** until Wave 1 golden app stays stable |
| **11** | Design system + assets | **A5** OG tokens, **`splash`**, advanced OG                                                                    |
...
Waves **1–6** are **✅ done** (detail below). Waves **9, 12** are **🟨 partial**; **7, 8, 10, 11** are **⬜** — see **[At a glance](#at-a-glance--waves-done-vs-left)** above. Full sequencing: [`Roadmap.md`](./Roadmap.md).
...
| **CLI TUI + crawl (9 / 12)** | Partial           | **`@better-seo/cli`**: Clack **launcher**, **`crawl robots` / `crawl sitemap`**, **`doctor`**, **`init`**, **`migrate`** — **`docs/commands.md`**. **L9–L11** templates TBD.                                                       |
...
| 9       | TUI, init, doctor      |   🟨   | **Clack TUI**, **`crawl`**, **`doctor` / `init` / `migrate`** hints — **L9–L11** templates / richer **L8** TBD                             |
| 10      | add / scan / fix       |   ⬜   | **L3/L4**                                                                                                                                  |
| 11      | Design system OG       |   ⬜   | **A5**, **`splash`** depth                                                                                                                 |
| 12      | Crawl + migrate        |   🟨   | **`better-seo-crawl`** + CLI **`crawl`** + **`fromNextSeo`**; RSS / sitemap index / codemod **migrate** TBD                                |

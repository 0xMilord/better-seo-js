...
| **`renderTags`**            | Wave 5 complete   | Canonical, hreflang, OG (**`og:url`**, **`og:type`**, **all `og:image`** entries + dims/alt), Twitter, JSON-LD; parity with **`toNextMetadata`** OG surface; **`render.test.ts`**.                                           |
...
| **Still not built**         | See waves 6+      | **`better-seo-crawl`**, full **`fromNextSeo`**, **`onRenderTags`** / **`extendChannels`**, rich **`initSEO`** inference (PRD §3.9); **Remix / Astro / Nuxt** adapters (docs: **`docs/adapters/future-frameworks.md`**).      |
...
| Monorepo build order                                      |   ✅   | Root **`package.json` `build`**: **core** → **assets** → **cli** → **next** → **react** → **nextjs-app** → **react-seo-vite-example**                               |
